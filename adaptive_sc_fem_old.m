...
fem = initialise_fem_matrices_new(problem,params,fem);
fprintf('done (%f seconds)\n', toc());
...
while t < params.T
    ...
    switch params.adapt_type
        ...
        case 'residual'
            E = max([pi_t_r, 1e-1]);
            adapt_spatial = pi_x_r > E;
            adapt_param = pi_y_r > E;
        ...
        switch params.adapt_type
            ...
            case 'residual'
                if adapt_spatial == 1
                    ...
                    fem = initialise_fem_matrices_new(problem,params,fem);
                    Z_I_star_new = initialise_approximation_structure(problem, params, fem, I_star);
                    for ii = 1:length(Z_I_star_new)
                        Z_I_star_new{ii}.u_z = Z_I_star{ii}.u_z(:,1);
                        Z_I_star_new{ii}.t_z = Z_I_star{ii}.t_z(1);
                        Z_I_star_new{ii}.u_z_tplusdt = Z_I_star{ii}.u_z(:,1);
                        Z_I_star_new{ii}.tplusdt = Z_I_star{ii}.t_z(1);
                    end
                    Z_I_star = Z_I_star_new;
                end
                if adapt_param == 1
                    %% Parametric
                    work = 0*eta_y_mi;

                    J = mark_I(eta_y_mi, work, eta_mi, E, params);
                    fprintf('marked indices...')
                    disp(J);
                    data_table = post_process(data_table, t + delta_t, delta_t, Z_I_star,  Z_I_star_lofi, ...
                        I_star, I, E, error, ...
