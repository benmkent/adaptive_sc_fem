function precompute = precompute_DyDy(Z_I_star, fem, problem, precompute)    
...
%     precompute.uQv = uQv;
    ...
    for ii = 1:n_knots
        y_ii = [Z_I_star{ii}.z];
        y_diff_ii = [1; y_ii((nConv+1):(nConv+nDiff))];
%         u_z_tplusdt_ii = u_z_tplusdt(:,ii);
        for jj = 1:n_knots
            y_jj = [1; Z_I_star{jj}.z];
            y_diff_jj = [1; y_jj((nConv+1):(nConv+nDiff))];
...
%             u_z_tplusdt_jj = u_z_tplusdt(:,jj);
            ...
            for kk=1:(nDiff+1)
                for ll=1:(nDiff+1)
                    DD_iijj = DD_iijj + y_diff_ii(kk) * y_diff_jj(ll) * fem.a_pairs{kk,ll};
                end
            end
            DyDy{ii,jj} = DD_iijj;
