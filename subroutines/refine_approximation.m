function [Z_I_star_refined, I_new ,I_star_new] = refine_approximation(tr, Z_I_star, I, I_star, J, problem, params, fem, hifi)
%REFINE_APPROXIMATION Refine SC_FEM approximation with addition MIs
%
%   Inputs  tr                  Current sync time
%           Z_I_star            Current approximation data structure
%           I                   Sparse grid
...
%           fem                 FEM data
%           hifi                Flag for hi-fidelity timestepping
%   Outputs Z_I_star_refined    Updated approximation data structure
%           I_new               Updated sparse grid
%           I_star_new          Updated enhanced sparse grid
...
Z_I_star_refined = initialise_approximation_structure(params, fem, I_star_new);

% Copy timestepping data for old points into appropriate places
for ii = 1:length(old_pts);
    Z_I_star_refined{old_pts(ii)}.u_z = Z_I_star{old_pts_in_I_star(ii)}.u_z;
    Z_I_star_refined{old_pts(ii)}.u_z_tplusdt = Z_I_star{old_pts_in_I_star(ii)}.u_z_tplusdt;
    Z_I_star_refined{old_pts(ii)}.t_z = Z_I_star{old_pts_in_I_star(ii)}.t_z;
end

% Assuming we wish to adapt through interpolation new collocation points
% are initialised at the time t_{r}
