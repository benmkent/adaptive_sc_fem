...
function GERK(F, NEQN, Y, T, TOUT, RELERR, ABSERR, IFLAG, GERROR, WORK, IWORK)
% 
...
% C *******************************************************************
     GERKS(F, NEQN, Y, T, TOUT, RELERR, ABSERR, IFLAG,...
...
function GERKS(F, NEQN, Y, T, TOUT, RELERR, ABSERR, IFLAG,...      GER   10
            GERROR, YP, H, F1, F2, F3, F4, F5, YG, YGP, SAVRE, SAVAE,...
            NFE, KOP, INIT, JFLAG, KFLAG)
% C      FEHLBERG FOURTH(FIFTH) ORDER RUNGE-KUTTA METHOD WITH
% C      GLOBAL ERROR ASSESSMENT
...
%       IF (NEQN.LT.1) GO TO 10
        if NEQN < 1; IFLAG = 7; return; end
%       IF ((RELERR.LT.0.) .OR. (ABSERR.LT.0.)) GO TO 10
        if RELERR < 0 || ABSERR < 0 ; IFLAG = 7; return; end
%       MFLAG = IABS(IFLAG)
        MFLAG = abs(IFLAG);
%       IF ((MFLAG.GE.1) .AND. (MFLAG.LE.7)) GO TO 20
        if MFLAG >=1 && MFLAG <=7
                if MFLAG > 1 % Not first call
%                     GOTO70()
                end
                if T == TOUT; IFLAG =7; return; end;
                if MFLAG ~= 2;
                    %GOTO 30;
                    if IFLAG == 3;
                            %GOTO 50;
...
function FEHL(F, NEQN, Y, T, H, YP, F1, F2, F3, F4, F5, S) %     FEH   10
% 
